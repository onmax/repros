...
This folder uses `pnpm patch` to patch `@ai-sdk/google@3.0.80` so Vertex-native requests omit `functionCall.id` and `functionResponse.id` while preserving AI SDK `toolCallId` semantics internally.
