# Devframe replay buffer eviction: patched control
...
The command confirms the patched behavior and exits zero. `corepack pnpm verify --expect-fixed` also passes here. The same assertion fails in the published sibling.
