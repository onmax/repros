# Better Auth session throw fixed control

Given `@nuxtjs/better-auth` 0.2.5 and Better Auth 1.7.1, a successful get-session request should populate auth state when fetchOptions.throw is true, but both state values become null.
...
Reproduction branch: `repro/better-auth-session-throw`. Clone with `git clone --branch repro/better-auth-session-throw --single-branch https://github.com/onmax/repros.git`, then enter this fixture directory. The sibling failing fixture uses the same command.
