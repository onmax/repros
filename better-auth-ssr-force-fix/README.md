# Better Auth ssr force fixed control

Given `@nuxtjs/better-auth` 0.2.5 and Better Auth 1.7.1, fetchSession({ force: true }) during SSR should request uncached data, but the cache-bypass query is omitted.
...
Reproduction branch: `repro/better-auth-ssr-force`. Clone with `git clone --branch repro/better-auth-ssr-force --single-branch https://github.com/onmax/repros.git`, then enter this fixture directory. The sibling failing fixture uses the same command.
