...
This failing fixture demonstrates a TypeScript contract bug in `@nuxtjs/better-auth` 0.2.5. With Nuxt 4.5.2, the module's generated declarations allow an invalid HTTP method, a string `immediate` option, and a nonexistent method on the returned `AsyncData` object. These calls are unrelated to authentication.

Given the pinned packages and `probe.ts`, running the compiler should reject all three calls. This failing fixture accepts all three invalid calls.
...
cd repros/better-auth-fetch-types
