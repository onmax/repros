...
The causal change is the committed pnpm patch to `@vue/runtime-core@3.5.42`. It preserves the block's `hasOnce` marker across updates and the vnode's cache index across cloning, then clears the lexical owner's cache during teardown only when it belongs to the instance being removed. The patched CJS development, CJS production, and ESM bundler files match the builds of the upstream source change at [commit `a39e80e`](https://github.com/onmax/vue-core/commit/a39e80e8e7516a884a0338a1ff2ee2ed5ce273e9), based on Vue commit `d105e19e914bca6e1140bfec074e055ed0679e21`. This verifier exercises the CJS development build; it does not establish production or browser behavior.
