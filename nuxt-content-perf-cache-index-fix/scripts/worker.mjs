...
let cacheInserts = 0
const perf = globalThis.__contentPerf = { phase: 'startup', sql: [], expectedChecksums: {}, variant, docs }
const record = (sql, db, execute, params = []) => {
  ...
  const entry = { type, phase: perf.phase, inTransaction: db.inTransaction, key: type === 'cache-DELETE' ? params[0] : sql.match(/content\/report-\d+\.md/)?.[0] }
...
try {
  ...
  const cacheBoundaries = variant.fixed && variant.kind === 'cache-writes' && perf.parsedFilesCount > 0
    ? ['BEGIN', 'COMMIT'].map(type => perf.sql.find(row => row.phase === 'process' && row.type === type)).filter(Boolean) : []
  perf.cacheWrites = {
