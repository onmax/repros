"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.default = void 0;var _promises = await jitiImport("node:fs/promises");
...
var _chokidar = _interopRequireDefault(await jitiImport("chokidar"));
var _tinyglobby = await jitiImport("tinyglobby");
var _lib = await jitiImport("./db/lib");
var _nodeFs = await jitiImport("node:fs");
var _hookable = await jitiImport("hookable");
var _nodePath = await jitiImport("node:path");
var _nodeUrl = await jitiImport("node:url");
var _getPortPlease = await jitiImport("get-port-please");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}

const version = "0.10.7";

const log$5 = _kit.logger.withTag("nuxt:hub");
...
const { resolve, resolvePath } = (0, _kit.createResolver)("file:///Users/maxi/repros/nuxthub-870-fix/node_modules/.pnpm/@nuxthub+core@0.10.7_patch_hash=704b5584f1b70393561ea21e1cad99e718b3dd8fc3f85bab16d0bbf_60a4629755dffb1abb0d7faf78c790e4/node_modules/@nuxthub/core/dist/module.mjs");
...
function resolveCacheConfig(hub) {
  ...
  if (userConfig.driver) {
    if (userConfig.driver === "cloudflare-kv-binding") {
      return (0, _defu.defu)(userConfig, { binding: "CACHE" });
    }
    return userConfig;
  }
  if (hub.hosting.includes("cloudflare")) {
    return (0, _defu.defu)(userConfig, {
  ...
  return (0, _defu.defu)(userConfig, {
    driver: "fs-lite",
    base: (0, _pathe.join)(hub.dir, "cache")
  });
}
async function setupCache(nuxt, hub, _deps) {
  ...
  nuxt.options.nitro.storage.cache = (0, _defu.defu)(nuxt.options.nitro.storage.cache, cacheStorageConfig);
  if (nuxt.options.dev) {
    nuxt.options.nitro.devStorage ||= {};
    nuxt.options.nitro.devStorage.cache = (0, _defu.defu)(nuxt.options.nitro.devStorage.cache, cacheStorageConfig);
  }
  logWhenReady(nuxt, `\`hub:cache\` using \`${cacheConfig.driver.split("/").pop()}\` driver`);
}

const log$4 = _kit.logger.withTag("nuxt:hub");
const cloudflareHooks = (0, _hookable.createHooks)();
function setupCloudflare(nuxt, hub) {
  ...
  if (!hub.hosting.includes("pages")) {
    nuxt.options.nitro.cloudflare.wrangler = (0, _defu.defu)(nuxt.options.nitro.cloudflare.wrangler, {
  ...
  if (nuxt.options.dev) {
    ...
    nuxt.hook("modules:done", async () => {
      const wranglerConfig = nuxt.options.nitro.cloudflare?.wrangler;
      if (!wranglerConfig || nuxt.options.nitro.cloudflare.dev?.configPath) {
        return;
      }
      const configPath = (0, _pathe.join)(nuxt.options.buildDir, "wrangler.json");
      await (0, _promises.mkdir)(nuxt.options.buildDir, { recursive: true });
      await (0, _promises.writeFile)(configPath, JSON.stringify({
        compatibility_date: nuxt.options.compatibilityDate,
        ...wranglerConfig
      }, null, 2), "utf-8");
...
async function processWranglerConfigFile(nuxt, targetEnv) {
  const wranglerPath = (0, _pathe.join)(nuxt.options.rootDir, ".output", "server", "wrangler.json");
  if (!(0, _nodeFs.existsSync)(wranglerPath)) {
    log$4.warn(`No wrangler.json found at ${wranglerPath}, skipping wrangler processing`);
    return;
  }
  try {
    const content = await (0, _promises.readFile)(wranglerPath, "utf-8");
    ...
    await (0, _promises.writeFile)(wranglerPath, JSON.stringify(config, null, 2), "utf-8");
  } catch (error) {
    log$4.error(`Failed to process wrangler config: ${error}`);
  }
}

const log$3 = _kit.logger.withTag("nuxt:hub");
...
async function resolveDatabaseConfig(nuxt, hub) {
  ...
  config = (0, _defu.defu)(config, {
    migrationsDirs: (0, _kit.getLayerDirectories)(nuxt).map((layer) => (0, _pathe.join)(layer.server, "db/migrations")),
    queriesPaths: [],
  ...
  switch (config.dialect) {
    case "sqlite":{
        ...
        if (process.env.TURSO_DATABASE_URL && process.env.TURSO_AUTH_TOKEN) {
          ...
          config.connection = (0, _defu.defu)(config.connection, {
        ...
        if (userExplicitLibsql) {
          config.connection = (0, _defu.defu)(config.connection, { url: "" });
          break;
        }
        ...
        config.driver ||= "libsql";
        config.connection = (0, _defu.defu)(config.connection, { url: `file:${(0, _pathe.join)(hub.dir, "db/sqlite.db")}` });
        await (0, _promises.mkdir)((0, _pathe.join)(hub.dir, "db"), { recursive: true });
        break;
      }
    case "postgresql":{
        ...
        config.connection = (0, _defu.defu)(config.connection, { url: process.env.POSTGRES_URL || process.env.POSTGRESQL_URL || process.env.DATABASE_URL || "" });
        ...
        config.connection = (0, _defu.defu)(config.connection, { dataDir: (0, _pathe.join)(hub.dir, "db/pglite") });
        await (0, _promises.mkdir)((0, _pathe.join)(hub.dir, "db/pglite"), { recursive: true });
        break;
      }
    case "mysql":{
        if (hub.hosting.includes("cloudflare") && config.connection?.hyperdriveId && !config.driver) {
        ...
        config.driver ||= "mysql2";
        config.connection = (0, _defu.defu)(config.connection, { uri: process.env.MYSQL_URL || process.env.DATABASE_URL || "" });
...
async function setupDatabase(nuxt, hub, deps) {
  ...
  (0, _kit.addServerPlugin)(resolve("db/runtime/plugins/migrations.dev"));
  nuxt.hook("modules:done", async () => {
    await generateDatabaseSchema(nuxt, hub);
    await nuxt.callHook("hub:db:migrations:dirs", migrationsDirs);
    await (0, _lib.copyDatabaseMigrationsToHubDir)(hub);
    await nuxt.callHook("hub:db:queries:paths", queriesPaths, dialect);
    await (0, _lib.copyDatabaseQueriesToHubDir)(hub);
  });
  nuxt.hook("nitro:build:public-assets", async (nitro) => {
    await (0, _lib.copyDatabaseAssets)(nitro, hub);
    await (0, _lib.applyBuildTimeMigrations)(nitro, hub);
  });
  if (driver === "d1") {
...
async function generateDatabaseSchema(nuxt, hub) {
  ...
  const getSchemaPaths = async () => {
    const schemaPatterns = (0, _kit.getLayerDirectories)(nuxt).map((layer) => [
    (0, _pathe.resolve)(layer.server, "db/schema.ts"),
    (0, _pathe.resolve)(layer.server, `db/schema.${dialect}.ts`),
    (0, _pathe.resolve)(layer.server, "db/schema/*.ts")]
    ).flat();
    let schemaPaths2 = await (0, _tinyglobby.glob)(schemaPatterns, { absolute: true, onlyFiles: true });
    await nuxt.callHook("hub:db:schema:extend", { dialect, paths: schemaPaths2 });
    schemaPaths2 = schemaPaths2.filter((path) => {
      const meta = (0, _lib.getDatabaseSchemaPathMetadata)(path);
  ...
  if (nuxt.options.dev && !nuxt.options._prepare) {
    const watchDirs = (0, _kit.getLayerDirectories)(nuxt).map((layer) => (0, _pathe.resolve)(layer.server, "db"));
    const watcher = _chokidar.default.watch(watchDirs, {
      ignoreInitial: true
    });
    watcher.on("all", async (event, path) => {
      ...
      log$3.info(`Database schema ${event === "add" ? "added" : event === "unlink" ? "removed" : "changed"}: \`${(0, _pathe.relative)(nuxt.options.rootDir, path)}\``);
      log$3.info("Make sure to run `npx nuxt db generate` to generate the database migrations.");
      schemaPaths = await getSchemaPaths();
      await (0, _kit.updateTemplates)({ filter: (template) => template.filename.includes("hub/db/schema.entry.ts") });
      await (0, _lib.buildDatabaseSchema)(nuxt.options.buildDir, { relativeDir: nuxt.options.rootDir, alias: nuxt.options.alias });
      const physicalDbDir = (0, _pathe.join)(nuxt.options.rootDir, "node_modules", "@nuxthub", "db");
      try {
        await (0, _promises.copyFile)((0, _pathe.join)(nuxt.options.buildDir, "hub/db/schema.mjs"), (0, _pathe.join)(physicalDbDir, "schema.mjs"));
        await (0, _promises.copyFile)((0, _pathe.join)(nuxt.options.buildDir, "hub/db/schema.d.mts"), (0, _pathe.join)(physicalDbDir, "schema.d.mts"));
      } catch (error) {
      }
    });
  ...
  nuxt.hooks.hookOnce("app:templatesGenerated", async () => {
    await (0, _lib.buildDatabaseSchema)(nuxt.options.buildDir, { relativeDir: nuxt.options.rootDir, alias: nuxt.options.alias });
    const physicalDbDir = (0, _pathe.join)(nuxt.options.rootDir, "node_modules", "@nuxthub", "db");
    await (0, _promises.mkdir)(physicalDbDir, { recursive: true });
    try {
      await (0, _promises.copyFile)((0, _pathe.join)(nuxt.options.buildDir, "hub/db/schema.mjs"), (0, _pathe.join)(physicalDbDir, "schema.mjs"));
      const buildDirSource = (0, _pathe.join)(nuxt.options.buildDir, "hub/db/schema.d.mts");
      const nuxtDirSource = (0, _pathe.join)(nuxt.options.rootDir, ".nuxt/hub/db/schema.d.mts");
      let schemaTypes;
      try {
        schemaTypes = await (0, _promises.readFile)(buildDirSource, "utf-8");
      } catch {
        try {
          schemaTypes = await (0, _promises.readFile)(nuxtDirSource, "utf-8");
        } catch {
        }
      }
      if (schemaTypes && schemaTypes.length > 50) {
        await (0, _promises.writeFile)((0, _pathe.join)(physicalDbDir, "schema.d.mts"), schemaTypes);
      } else if (!nuxt.options.test) {
        await (0, _promises.writeFile)((0, _pathe.join)(physicalDbDir, "schema.d.mts"), `export * from './schema.mjs'`);
      }
    } catch (error) {
...
${hasReplicas ? `    const primary = drizzle({ connection: { uri }, schema${modeOption}${casingOption} })
  ...
  await (0, _promises.writeFile)(
    (0, _pathe.join)(physicalDbDir, "db.mjs"),
...
`;
  await (0, _promises.writeFile)(
    (0, _pathe.join)(physicalDbDir, "db.d.ts"),
    physicalDbTypes
  );
  const packageJson = {
    name: "@nuxthub/db",
  ...
  try {
    await (0, _promises.writeFile)((0, _pathe.join)(physicalDbDir, "package.json"), JSON.stringify(packageJson, null, 2));
    const schemaPath = (0, _pathe.join)(physicalDbDir, "schema.mjs");
    const schemaDtsPath = (0, _pathe.join)(physicalDbDir, "schema.d.mts");
    const schemaExists = await (0, _promises.stat)(schemaPath).then((s) => s.size > 20).catch(() => false);
    const schemaDtsExists = await (0, _promises.stat)(schemaDtsPath).then((s) => s.size > 20).catch(() => false);
    if (!schemaExists) await (0, _promises.writeFile)(schemaPath, "export {}");
    if (!schemaDtsExists) await (0, _promises.writeFile)(schemaDtsPath, "export {}");
  } catch (error) {
    throw new Error(`Failed to create @nuxthub/db package files: ${error.message}`);
  }
  nuxt.options.alias["hub:db"] = "@nuxthub/db";
  (0, _kit.addServerImports)({ name: "db", from: "@nuxthub/db", meta: { description: `The ${driver} database client.` } });
  (0, _kit.addServerImports)({ name: "schema", from: "@nuxthub/db", meta: { description: `The database schema object` } });
}
...
export default defineConfig({
  ...
  schema: '${(0, _pathe.relative)(nuxt.options.rootDir, resolve(nuxt.options.buildDir, "hub/db/schema.mjs"))}',
  out: '${(0, _pathe.relative)(nuxt.options.rootDir, resolve(nuxt.options.rootDir, `server/db/migrations/${dialect}`))}'
});`
  });
}
...
function resolveKVConfig(hub) {
  ...
  if (typeof hub.kv === "object" && "driver" in hub.kv) {
    if (hub.kv.driver === "cloudflare-kv-binding") {
      return (0, _defu.defu)(hub.kv, { binding: "KV" });
    }
    return hub.kv;
  }
...
async function setupKV(nuxt, hub, deps) {
  ...
  nuxt.options.nitro.storage ||= {};
  nuxt.options.nitro.storage.kv = (0, _defu.defu)(nuxt.options.nitro.storage.kv, kvStorageConfig);
...
`;
  const physicalKvDir = (0, _pathe.join)(nuxt.options.rootDir, "node_modules", "@nuxthub", "kv");
  await (0, _promises.mkdir)(physicalKvDir, { recursive: true });
  await (0, _promises.writeFile)(
    (0, _pathe.join)(physicalKvDir, "kv.mjs"),
    kvContent
  );
  await (0, _promises.copyFile)(
    resolve("kv/runtime/kv.d.ts"),
    (0, _pathe.join)(physicalKvDir, "kv.d.ts")
  );
  const packageJson = {
    name: "@nuxthub/kv",
    version: "0.0.0",
  ...
  await (0, _promises.writeFile)(
    (0, _pathe.join)(physicalKvDir, "package.json"),
    JSON.stringify(packageJson, null, 2)
  );
  nuxt.options.alias["hub:kv"] = "@nuxthub/kv";
  (0, _kit.addServerImports)({ name: "kv", from: "@nuxthub/kv", meta: { description: `The Key-Value storage instance.` } });
  (0, _kit.addTypeTemplate)({
...
const log$2 = _kit.logger.withTag("nuxt:hub");
...
function resolveBlobConfig(hub, deps) {
  ...
  return (0, _defu.defu)(hub.blob, {
    driver: "fs",
    dir: (0, _pathe.join)(hub.dir, "blob")
  });
}
async function setupBlob(nuxt, hub, deps) {
  ...
  (0, _kit.addImportsDir)(resolve("blob/runtime/app/composables"));
...
`;
  const physicalBlobDir = (0, _pathe.join)(nuxt.options.rootDir, "node_modules", "@nuxthub", "blob");
  await (0, _promises.mkdir)(physicalBlobDir, { recursive: true });
  await (0, _promises.writeFile)((0, _pathe.join)(physicalBlobDir, "blob.mjs"), blobContent);
  await (0, _promises.copyFile)(
    resolve("blob/runtime/blob.d.ts"),
    (0, _pathe.join)(physicalBlobDir, "blob.d.ts")
  );
  const packageJson = {
    name: "@nuxthub/blob",
    version: "0.0.0",
  ...
  await (0, _promises.writeFile)((0, _pathe.join)(physicalBlobDir, "package.json"), JSON.stringify(packageJson, null, 2));
  nuxt.options.alias["hub:blob"] = "@nuxthub/blob";
  (0, _kit.addServerImports)({ name: "blob", from: "@nuxthub/blob", meta: { description: `The Blob storage instance.` } });
  (0, _kit.addServerImports)({ name: "ensureBlob", from: "@nuxthub/blob", meta: { description: `Ensure the blob is valid and meets the specified requirements.` } });
  (0, _kit.addTypeTemplate)({
...
let promise = null;
let port = 4983;
const log$1 = _kit.logger.withTag("nuxt:hub");
async function launchDrizzleStudio(nuxt, hub) {
  ...
  port = await (0, _getPortPlease.getPort)({ port: 4983 });
  try {
    const { dialect, driver, connection } = dbConfig;
    const dbEntry = (0, _nodePath.resolve)(nuxt.options.rootDir, "node_modules", "@nuxthub", "db", "db.mjs");
    if (!(0, _nodeFs.existsSync)(dbEntry)) {
      throw new Error(`Cannot find @nuxthub/db at ${dbEntry}. Run \`nuxi prepare\` to generate it.`);
    }
    const { schema } = await ((specifier) => new Promise((r) => r(`${specifier}`)).then((s) => jitiImport(s).then((m) => _interopRequireWildcard(m))))((0, _nodeUrl.pathToFileURL)(dbEntry).href);
    ...
    if (dialect === "postgresql") {
      if (driver === "pglite") {
        ...
        const { startStudioPostgresServer } = await Promise.resolve().then(() => jitiImport('drizzle-kit/api').then((m) => _interopRequireWildcard(m)));
        log$1.info(`Launching Drizzle Studio with PostgreSQL...`);
        await startStudioPostgresServer(schema, connection, { port });
      }
    } else if (dialect === "mysql") {
      const { startStudioMySQLServer } = await Promise.resolve().then(() => jitiImport('drizzle-kit/api').then((m) => _interopRequireWildcard(m)));
      ...
      if (driver === "d1") {
        ...
        const { startStudioSQLiteServer } = await Promise.resolve().then(() => jitiImport('drizzle-kit/api').then((m) => _interopRequireWildcard(m)));
...
const log = _kit.logger.withTag("nuxt:hub");
const module$1 = exports.default = (0, _kit.defineNuxtModule)({
  meta: {
    name: "@nuxthub/core",
    configKey: "hub",
    version,
  ...
  async setup(options, nuxt) {
    ...
    const hosting = process.env.NITRO_PRESET || nuxt.options.nitro.preset || _stdEnv.provider;
    const hub = (0, _defu.defu)(options, {
      // Local storage
      dir: ".data",
      hosting,
      // NuxtHub features
    ...
    hub.dir = await (0, _pathe.resolve)(nuxt.options.rootDir, hub.dir);
    await (0, _promises.mkdir)(hub.dir, { recursive: true }).catch((e) => {
      if (e.errno !== -17) throw e;
    });
    const packageJSON = await (0, _pkgTypes.readPackageJSON)(nuxt.options.rootDir);
    ...
    if (nuxt.options.dev) {
      const workspaceDir = await (0, _pkgTypes.findWorkspaceDir)(rootDir);
      const gitignorePath = (0, _pathe.join)(workspaceDir, ".gitignore");
      const gitignore = await (0, _promises.readFile)(gitignorePath, "utf-8").catch(() => "");
      const relativeDir = (0, _pathe.relative)(workspaceDir, hub.dir);
      if (!gitignore.includes(relativeDir)) {
        await (0, _promises.writeFile)(gitignorePath, `${gitignore ? gitignore + "\n" : gitignore}${relativeDir}`, "utf-8");
      }
    }
  }
}); /* v9-74b4082b90cd7636 */
