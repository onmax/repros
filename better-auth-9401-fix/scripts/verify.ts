...
try {
  ...
  const fixed = result.beforeReactiveEmail === 'admin318@example.com'
    && result.beforeFreshEmail === 'admin318@example.com'
    && result.reactiveEmail === 'user318@example.com'
    && result.freshEmail === 'user318@example.com'

  if (!fixed)
    throw new Error('Failed to revalidate useSession after impersonation')
}
finally {
